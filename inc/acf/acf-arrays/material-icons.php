...
$files = glob(get_template_directory() . '/src/assets/icons/social/*.svg');
foreach ($files as $icon) {
    // Get the contents of the current $icon
    $icon_content = file_get_contents($icon);

    // Output the current $icon

    $style = is_admin() ? 'style="width: 24px; height: 24px;"' : '';

    $icons[] = '<div class="li-icon" ' . $style . '>' . $icon_content . '</div>';
}
